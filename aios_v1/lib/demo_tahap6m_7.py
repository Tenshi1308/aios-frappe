"""
Demo Interaktif Sub-tahap 6M.7: Manufacturing Management Skills (Live LLM Inference).
Menampilkan 7 SOP Alur Kerja Manufaktur terdistribusi di 4 Job Roles, Simulasi RBAC, dan Pemanggilan LLM Nyata dengan SOP Markdown + Tool Calling.
Dijalankan via: `bench --site aios.localhost execute aios_v1.lib.demo_tahap6m_7.run_demo`
"""
...
def run_demo():
    print("\n" + "=" * 80)
    print("🏭 DEMO SUB-TAHAP 6M.7: MANUFACTURING MANAGEMENT SKILLS (LIVE LLM)")
    print("=" * 80)
    ...
    # 2. Meninjau 7 File SOP Manufacturing
    all_skills = load_all_skills(force_reload=True)
    mfg_skills = {k: v for k, v in all_skills.items() if v.get("branch") == "manufacturing"}

    print(f"\n[2] MENINJAU {len(mfg_skills)} SOP MANUFACTURING (skills/manufacturing/<job_role>/):\n")
    for idx, (slug, skill) in enumerate(mfg_skills.items(), 1):
    ...
    print("=" * 80)
    print("[3] SIMULASI PEMBAGIAN HAK AKSES PERAN KERJA SUB-AGENT (RBAC MANUFACTURING OPSI C):")

    mfg_roles = [
        ("production_planner", "📐 Production Planner (MRP, BOM & Biaya Produksi)"),
        ("production_scheduler", "⏱️ Production Scheduler (Jadwal MO & Kapasitas Mesin)"),
        ("production_supervisor", "🛠️ Production Supervisor (Shop Floor & OEE Mesin)"),
        ("production_manager", "👑 Production Manager (Tata Kelola Pabrik & Line Balancing)")
    ]

    for role_key, label in mfg_roles:
        skills = get_skills_for_worker(branch="manufacturing", worker_key=role_key)
    ...
    # 4. Merakit System Prompt & Tools untuk Peran Production Supervisor
    branch = "manufacturing"
    role = "production_supervisor"
    base_prompt = "Anda adalah Production Supervisor di AIOS. Pantau efektivitas mesin (OEE), konfirmasi hasil produksi, dan laporkan insiden downtime sesuai SOP."
    ...
    print("\n" + "=" * 80)
    print(f"[4] MEMUAT KONTEKS SUB-AGENT PRODUCTION SUPERVISOR:")
    ...
    # 5. Skenario Live LLM: Analisis OEE Mesin Pabrik
    user_query = "Mesin CNC Milling beroperasi dengan waktu terencana 480 menit dan waktu aktual operasi 432 menit. Waktu siklus standar idealnya adalah 0.5 menit per unit. Total output yang dihasilkan adalah 800 unit dengan 780 unit berkualitas baik tanpa cacat. Tolong hitung metrik OEE (Availability, Performance, Quality, Overall OEE), kategorinya terhadap standar World Class, dan berikan evaluasi sesuai SOP."
    print("\n" + "=" * 80)
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("🎉 DEMO 6M.7 SELESAI: Production Supervisor Berhasil Menerapkan SOP via Live LLM!")
        print("=" * 80 + "\n")
        return {"status": "SUCCESS", "phase": "6M.7", "mfg_skills": len(mfg_skills), "model": cfg["model"]}
